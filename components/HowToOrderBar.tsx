    ...
    <section id="how-to-order" className="bg-surface-container-low rounded-3xl p-6 sm:p-10 border border-outline-variant/40 space-y-8 shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 relative">
        {steps.map((s, idx) => {
          const Icon = s.icon;
          return (
            <ScrollReveal key={idx} delay={idx * 100} duration={600} direction="up" distance={24} className="h-full">
              <div
                className="h-full bg-surface-container-lowest p-5 rounded-2xl border border-outline-variant/40 space-y-3 relative group hover:border-secondary/40 hover:-translate-y-1.5 hover:shadow-lg transition-all duration-300 ease-out flex flex-col justify-between"
              >
